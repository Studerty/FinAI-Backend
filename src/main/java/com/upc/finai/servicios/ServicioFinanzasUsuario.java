...
public class ServicioFinanzasUsuario {
    ...
    public ResumenFinancieroDTO obtenerResumen(Long idUsuario) {
        ...
        List<MovimientoDTO> movimientos = movimientoRepositorio.findByUsuarioIdOrderByCreadoEnDesc(idUsuario)
                .stream()
    ...
    public MovimientoDTO crearMovimiento(Long idUsuario, CrearMovimientoDTO crearMovimientoDTO) {
        ...
        if (tipoMovimiento.equals("GASTO") && cuenta.getSaldo().compareTo(crearMovimientoDTO.getMonto()) < 0) {
    ...
    public List<MovimientoDTO> listarMovimientosPorTipo(Long idUsuario, String tipo) {
        ...
        return movimientoRepositorio.findByUsuarioIdAndTipoOrderByCreadoEnDesc(idUsuario, normalizarTipo(tipo))
                .stream()
                .map(movimiento -> modelMapper.map(movimiento, MovimientoDTO.class))
                .toList();
    }
    ...
    public List<PresupuestoDTO> listarPresupuestos(Long idUsuario) {
        obtenerUsuario(idUsuario);
        return presupuestoRepositorio.findByUsuarioIdOrderByCreadoEnDesc(idUsuario)
                .stream()
    ...
    public List<MetaAhorroDTO> listarMetas(Long idUsuario) {
        obtenerUsuario(idUsuario);
        return metaAhorroRepositorio.findByUsuarioIdOrderByFechaLimiteAsc(idUsuario)
                .stream()
    ...
    @Transactional
    public TicketSoporteDTO crearTicketSoporte(Long idUsuario, CrearTicketSoporteDTO crearTicketSoporteDTO) {
        Usuario usuario = obtenerUsuario(idUsuario);
    ...
    public List<TicketSoporteDTO> listarTicketsUsuario(Long idUsuario) {
        obtenerUsuario(idUsuario);
        return ticketSoporteRepositorio.findByUsuarioIdOrderByCreadoEnDesc(idUsuario)
                .stream()
